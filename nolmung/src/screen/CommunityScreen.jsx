...
import { getArticles_all, getArticles_friend, getArticles_qna, getArticles_region } from '../api/Article.js'
  ...
  const getArticle_all_func = async()=>{
    try{
      await getArticles_all(
        ...
        (err) => {
          console.log('Article_All err', err)
        }
      );
    } catch(err) {
      console.log(err);
    ...
    try {
      await getArticles_friend(
        { userId : 1 },
         ...
         (err) => {
          console.log("ArticleFriend Err", err)
         }
      )
    }catch(err){
      console.log(err);
